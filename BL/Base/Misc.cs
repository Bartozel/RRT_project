...
namespace BL.Base
{
    ...
    {
        public static double Distance(IPosition node1, IPosition node2)
        {
            return Math.Sqrt(Math.Pow(node1.XCoordinate - node2.XCoordinate, 2) + Math.Pow(node1.YCoordinate - node2.YCoordinate, 2));
        }

        //TODO test
        internal static (int newX, int newY) CalculateCloserPosition(ITreeNode staticNode, IPosition shiftNode, double currentDist, double maxDistance)
        {
