...
namespace Data
{
    public class SearchTree : ISearchedNodesTree
    {
        private ITreeNode[,] _tree;
        public int Count => _tree.Length;
        public ITreeNode Root { get; private set; }

        public SearchTree(int xRange, int yRange, IPosition rootPosition)
        {
            _tree = new ITreeNode[xRange, yRange];
            Root = new SearchTreeNode(rootPosition);
            Insert(Root);
        }
        ...
        {
            _tree = new SearchTreeNode[_tree.GetLength(0), _tree.GetLength(1)];
        }
